...
int ListSearch(List* l, const void* p) {
  ...
  // ------Search starts at current node
  /* Uncomment to see
  struct _ListNode* aux = l->current;
  int indexAux = l->currentPos;
  
  int comp = l->compare(aux->item, p);
  
  if(comp > 0){ //if element is smaller than current node
     aux = l->head;
     indexAux = 0;
     while(indexAux != l->currentPos){
          comp = l->compare(aux->item, p);
          if(comp == 0){
             ListMove(l,indexAux);
             return 0;
          }
  ...
  else if(comp < 0){ //if element is bigger than current node
     aux = aux->next;
     indexAux++;
     while(aux != NULL){
      comp = l->compare(aux->item, p);
  ...
  else      //if element is equal to the current
    return 0;
  //*/
}
