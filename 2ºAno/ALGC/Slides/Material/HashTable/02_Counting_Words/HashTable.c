...
int HashTableGet(HashTable* hashT, const char* key) {
...
int HashTablePut(HashTable* hashT, const char* key, int value) {
  int result = _searchHashTable(hashT, key);
...
int HashTableReplace(const HashTable* hashT, const char* key, int value) {
  int index = _searchHashTable(hashT, key);
  if (index == -1 || hashT->table[index].isFree == 1) {
    // NOT FOUND
    return 0;
