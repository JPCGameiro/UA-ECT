...
int BSTreeGetHeight(const BSTree* root) {
  ...
  if(BSTreeGetHeight(root->left) > BSTreeGetHeight(root->right))
	return 1 + BSTreeGetHeight(root->left);
  return 1 + BSTreeGetHeight(root->right);
}

//Get Min Tree value Iterative
...
//Traverse NLR (node left right)
void BSTreeTraverseInPREOrder(BSTree* root, void (*function)(ItemType* p)) {
  if(root == NULL) return;
  function(&root->item);
  if(root->left!=NULL) BSTreeTraverseInPREOrder(root->left, function);
  if(root->right!=NULL) BSTreeTraverseInPREOrder(root->right, function);
}

//Traverse LNR (left node right)
void BSTreeTraverseINOrder(BSTree* root, void (*function)(ItemType* p)) {
  if(root == NULL) return;
  if(root->left!=NULL) BSTreeTraverseINOrder(root->left, function);
  function(&root->item);
  if(root->right!=NULL) BSTreeTraverseINOrder(root->right, function);
}

//Traverse LRN (left node right)
void BSTreeTraverseInPOSTOrder(BSTree* root, void (*function)(ItemType* p)) {
  if(root == NULL) return;
  if(root->left!=NULL) BSTreeTraverseInPOSTOrder(root->left, function);
  if(root->right!=NULL) BSTreeTraverseInPOSTOrder(root->right, function);
  function(&root->item);  
}
