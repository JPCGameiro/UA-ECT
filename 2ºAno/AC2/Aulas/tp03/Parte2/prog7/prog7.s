...
while:					#while(1){
	...
	andi $s1, $s1, 0x0002		#	extrair bit 2
	
	bne  $s1, 0x0002, right		#	if(rb2=1){
					
	andi $s3, $s2, 0x8		#		extrair o bit mais significativo de cnt
	sll  $s2, $s2, 1		#		cnt = cnt<<1;
	andi $s2, $s2, 0xFFFE		#		manter todos os bits excepto o menos significativo
if1:	bne $s3, 0x8, if0		#		if(bit3 = 1)
	ori $s2, $s2, 0x0001		#			cnt[bit0]=1;
	j   end				#		else
if0:	ori $s2, $s2, 0x0000		#			cnt[bit0]=0
end:    j   endr			#	}
					#	else if(rb2=0){
right:					
	andi $s3, $s2, 0x1		#		extrair o bit menos significativo de cnt
	srl  $s2, $s2, 1		#		cnt = cnt>>1;
	andi $s2, $s2, 0xFFF7		#		manter todos os bits excepto o mais significativo
if1r:	bne $s3, 0x1, if0r		#		if(bit0 = 1)
	ori $s2, $s2, 0x0008		#			cnt[bit3]=1;
	j   endr			#		else
if0r:	ori $s2, $s2, 0x0000		#			cnt[bit3]=0
endr:					#	}
	...
	li  $a0, 333			#	delay(667); frequencia de 3Hz
	jal delay			#
	j   while			#}
