...
class Root(object):
	...
	def list(self, names=None, name=None, nome=None ,color=None, detected=None):
		...
		if(names!=None):
			...
			cnt = len(lstimn)         #contador que vai servir para que a extração dos objetos ocorra o número correto de vezes
			cntcol = len(lstimn)      #contador que vai seevir para que a deteção da cor ocorra o número correto de vezes
	...
	def test(self):
		...
		res = db.execute("SELECT * FROM Imagem")
		return "ola"
		'''
		#Devolver todos os objetos detetados e outras imformações
		else:
			for obj in return_obj():
				jsono = jsono+'{"Objeto": "'+str(obj)+'", "original": "'+str(codif_img(lstimn[cnt1]))+'", "imagem": "'+str(codif_img(lstimo[cnt1]))+'", "confianca": "'+ str(lstcfn[cnt1])+'"}'
				cnt1=cnt1+1
			jsono = jsono + ']'
			jsono0 = json.loads(jsono)
			cherrypy.response.headers["Content-Type"] = "application/json"
